...
private:
    ...
    void triggerRandomEvent() {
        ...
        if (probofevent < 50) {
            ...
            int stockIndex = distribution(gen) % stocks.size();
            double eventImpact = Normdist(0, 20);
            cout << "Random Event: " << stocks[stockIndex].getName() << " is affected by " << eventImpact << endl;
            stocks[stockIndex].updatePrice(eventImpact);
        }
    }
};
    ...
    do {
        ...
        if (choice == '1') {
            ...
            if (stockIndex >= 0 && stockIndex < 3) {
                player.buyStock(market.getStock(stockIndex), quantity);
            } else {
            ...
            if (stockIndex >= 0 && stockIndex < 3) {
                player.sellStock(market.getStock(stockIndex), quantity);
            } 
            else {
                cout << "Invalid stock index\n";
            }
